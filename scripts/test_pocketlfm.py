...
def small_cfg(**kw) -> PocketLFMConfig:
    base = dict(d_model=64, n_layers=4, attn_layer_indices=(1, 3), n_heads=4, n_kv_heads=2,
                ff_dim=128, mtp_horizon=3, depth_dim=48, depth_layers=2, head_dim=32, head_depth=2,
                text_vocab=100, text_dim=32, text_heads=4, num_emotions=4, ref_pool_heads=2)
    base.update(kw)
...
def test_compute_losses_with_reference_and_emotion():
    cfg = small_cfg()
    model = PocketLFM(cfg)
    tt, tl, lat, ll = _batch(cfg, [(6, 10), (5, 8)])
    ref = torch.randn(2, 12, cfg.latent_dim)
    ref_lens = torch.tensor([12, 9])
    emotion = torch.tensor([1, 3])
    out = model.compute_losses(tt, tl, lat, ll, ref_latents=ref, ref_lens=ref_lens, emotion=emotion)
